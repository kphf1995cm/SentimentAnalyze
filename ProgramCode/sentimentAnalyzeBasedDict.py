...
def sentiAnalyzeBaseDictFromPos(lastPos,childDir,reviewDataSetName,reviewDataSetFileType,windowSize,posBounder,negBounder,sentScoreBounder,timeInterval=20):
    ...
    '''获得目标数据路径'''
    dstSavePath = saveResPath + '/' + childDir+reviewDataSetName +str(curTime)+ 'DA.txt'
    strangeWordPath=strangeWordDir+'/'+childDir+reviewDataSetName+str(curTime)+'DA.txt'
    '''获得原始数据'''
    review,curPos = tp.get_txt_data_from_pos(dataSetPath, "lines",lastPos)
    '''得到每句评论[[PosSum, NegSum],[],]'''
    if len(review)>0:
        sentiment_score_list=get_review_set_sentiement_score(review)
        '''得到每句评论的整体得分'''
        ...
        #outputStrangeWordPosInTxt(finalStrangeWordPos, dstSavePath)
        # '''绘制情感曲线图'''
        sentimentLinePath = figDir + '/' + childDir + reviewDataSetName + str(curTime) + 'SCDA.png'
        posNegRatioPath = figDir + '/' + childDir + reviewDataSetName + str(curTime) + 'PNRDA.png'
...
def sentiAnalyzeBaseDictFromLastPosShowAllUI(oldSentValue,oldPosProbility,oldRawReview,lastPos,reviewDataSetDir,reviewDataSetName,reviewDataSetFileType,windowSize,posBounder,negBounder,sentScoreBounder,timeInterval=20):
    begin=time.clock()
...
def handleMutiRoomInfo(timeSize,childDirList,reviewDataSetNameList,reviewDataSetFileType,windowSize,posBounder,negBounder,sentScoreBounder,timeInterval=20):
    ...
    while True:
        ...
        for pos in range(len(reviewDataSetNameList)):
            lastPosList[pos]=sentiAnalyzeBaseDictFromPos(lastPosList[pos],childDirList[pos],reviewDataSetNameList[pos],reviewDataSetFileType,windowSize,posBounder,negBounder,sentScoreBounder,timeInterval)
